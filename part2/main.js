...
//     for(let i = 0; i < AvgReationArr.length; i++) {
//       dataInput[i].innerHTML = AvgReationArr[i];
//     }
...
const clockIndicator = document.getElementById('stick');
  ...
  let count = 0; 
    ...
    let timerId = setInterval(() => { 
      document.addEventListener('keydown', function(event) {
        if (event.code === 'Space' ) {
          if ( Qcounter === 0 ) {
            if ( shift > 510 && shift < 540 ) {
              ROquantity++;
              console.log('RO', shift);
              indexCount++;
            } else if ( shift == 540 ) {
              correctQuantity++;
              console.log('CORR', shift);
              indexCount++;
            } else if ( shift > 540 && shift < 570 ) {
              RZquantity++;
              console.log('RZ', shift);
              indexCount++;
            } 
            if ( shift > 510 && shift <= 570 ) {
              AvgReationArr.push(((shift-540)/3)*15);
            }
            console.log(AvgReationArr);
            Qcounter++;
          }
    ...
    count++;
    if ( count <= 130 ) {
      clockIndicator.setAttribute('style', `transform: rotate(${shift+3}deg) !important;`);
      shift = shift + 3;
    } else {
      clearInterval(timerId);
    }
...
document.addEventListener('keyup', function(event) {
  if (event.code === 'Space') {
    if ( flag === true ) {
      flag = false;
      Qcounter = 0;
      clockIndicator.style.transform = 'rotate(180deg)';
      start();
    }    
  }
})
...
function start () {
  ...
  /*------*/
  clockIndicator.setAttribute('style', 'transform: rotate(180px) !important; ');
